                ...
                child: CustomScrollView(
                  ...
                  slivers: [
                    ...
                    SliverTreeView.indexed(
                      ...
                      onItemTap: (IndexedTreeNode<AccoutingTreeModel> IndexNode){
                          if(IndexNode.isLeaf) {
                            setState(() {
                            VMSalesInvoice.AccountingTo_select_id=IndexNode.key;
                            VMSalesInvoice.AccountingTo_select_name=IndexNode.data!.name;
                            });
  ...
  void buildTree() {
    ...
    for (var root in allAccountingTree) {
      if(int.parse(root.branch_no)<10 ) {
        ...
        allAccountingBrach1.addAll(
            allAccountingTree.where((Node1) => int.parse(Node1.father_no )==
                int.parse(root.branch_no) && Node1.father_no.compareTo(Node1.branch_no) != 0));
        if (allAccountingBrach1.isNotEmpty) { //add  root without node
          ...
          for (var Node1 in allAccountingBrach1) {
            ...
            allAccountingBrach2.addAll(
                allAccountingTree.where((Node2) => Node2.father_no.compareTo(Node1.branch_no) ==0
            ...
            if (allAccountingBrach2.isNotEmpty) {
              for (var Node2 in allAccountingBrach2) {
                ...
                indexedTrrre.elementAt("${root.branch_no}.${Node1.branch_no}").addAll(IndexNode2);
                IndexNode3.clear();
                allAccountingBrach3.clear();
                allAccountingBrach3.addAll(
                    allAccountingTree.where((Node3) => Node3.father_no.compareTo(Node2.branch_no) ==0
                ...
                if (allAccountingBrach3.isNotEmpty) {
                  for (var Node3 in allAccountingBrach3) {
                    ...
                    indexedTrrre.elementAt("${root.branch_no}.${Node1.branch_no}.${Node2.branch_no}").addAll(IndexNode3);
                    IndexNode4.clear();
                    allAccountingBrach4.clear();
                    allAccountingBrach4.addAll(
                        allAccountingTree.where((Node4) => Node4.father_no.compareTo(Node3.branch_no) ==0
                    ...
                    if (allAccountingBrach3.isNotEmpty) {
                      for (var Node4 in allAccountingBrach4) {
                        ...
                        indexedTrrre.elementAt("${root.branch_no}.${Node1.branch_no}.${Node2.branch_no}.${Node3.branch_no}").addAll(IndexNode4);
                        IndexNode5.clear();
                        allAccountingBrach5.clear();
                        allAccountingBrach5.addAll(
                            allAccountingTree.where((Node5) => Node5.father_no.compareTo(Node4.branch_no) ==0
                                && Node5.father_no.compareTo(Node5.branch_no) != 0));
                      }
                    }
                  }
                }
