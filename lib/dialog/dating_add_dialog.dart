  ...
  void CheckDated(DateTime date,String room,String DoctorName) {
    ...
    if (DatesListUniq.contains(DateTime.utc(date.year, date.month, date.day))){
      ...
      DatesList.forEach((Dating) {
        DateTime Time_Start = DateTime.parse(Dating.dateStart);
        DateTime Time_End = DateTime.parse(Dating.dateEnd);
        /// Doctor Date
        if (DateTime.utc(Time_Start.year,Time_Start.month, Time_Start.day)==DateTime.utc(date.year,date.month, date.day)){
          /// Doctor Find
          if (Dating.doctorName == DoctorName) {
            ...
            if (Dating.place == room) {
              ...
              if (Time_Start.hour==9){
                picked.TimeRange timeRange=  picked.TimeRange(startTime:  TimeOfDay(hour: 15, minute: 00), endTime:  TimeOfDay(hour:21, minute:00));
                disabledListedTime!.add(timeRange);
              }else{
                picked.TimeRange timeRange=  picked.TimeRange(startTime:  TimeOfDay(hour: 9, minute: 00), endTime:  TimeOfDay(hour:15, minute:00));
                disabledListedTime!.add(timeRange);
              }
              ...
              allDatesList.forEach((Dating) {
                ...
                //              datePlace=room;
                //                _datePlace.currentState!.setState(() {
                //                datePlace=room;
                //                });
            ...
            if (Dating.place == room) {
              ...
              setState(() {
                picked.TimeRange timeRange = picked.TimeRange(
                    ...
                    endTime: TimeOfDay(
                        hour: Time_End.hour, minute: Time_End.minute));
